...
export async function xTweetResultToActivityPubNote(
  ...
  const attachment = tweetResult.mediaDetails ? await Promise.all(tweetResult.mediaDetails.map(async (mediaDetail) => {
  ...
  return {
    ...
    url:
      `https://x.com/${tweetResult.user.screen_name}/status/${tweetResult.id_str}`,
